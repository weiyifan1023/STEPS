...
def get_tokenizer(model_name_or_path, fast_tokenizer=True):
    if "llama" in model_name_or_path:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_name_or_path,  trust_remote_code=True)#fast_tokenizer=fast_tokenizer,
        tokenizer.pad_token = tokenizer.pad_token #tokenizer.eos_token
...
def save_hf_format(model, tokenizer, args, sub_folder=""):
    ...
    tokenizer.save_pretrained(output_dir)  # tokenizer.save_vocabulary(output_dir)
...
def save_zero_three_model(model_ema, global_rank, save_dir, zero_stage=0, tokenizer=None):
    zero_stage_3 = (zero_stage == 3)
    os.makedirs(save_dir, exist_ok=True)
    # 注释掉原有的单体文件名定义
    # WEIGHTS_NAME = "pytorch_model.bin"
    ...
    if not zero_stage_3:
        ...
        if global_rank == 0:
            # 使用 HF 的接口代替 torch.save
            model_to_save.save_pretrained(
                save_dir,
                state_dict=output_state_dict,
                max_shard_size="4GB",  # 设置分片大小
