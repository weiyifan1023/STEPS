...
def save_model(model, tokenizer, global_step, args, epoch=None):
    ...
    if epoch is None:
        ...
        if args.zero_stage == 3:
            save_zero_three_model(model, args.global_rank, os.path.join(args.output_dir, str(global_step)), args.zero_stage, tokenizer)
    else:
        if args.global_rank == 0:
            save_hf_format(model, tokenizer, args, f'epoch-{epoch}-'+str(global_step))
        if args.zero_stage == 3:
            save_zero_three_model(model, args.global_rank, os.path.join(args.output_dir, f'epoch-{epoch}-'+str(global_step)), args.zero_stage, tokenizer)
...
class TextDataset_V2(Dataset):
    ...
    def encode(self, text: str):
        if self.args.delete_first_token:  # 默认false
        ...
        if 'qwen' in self.args.model_name_or_path.lower():
            ...
            for midx, message in enumerate(messages):
                if message['role'].lower() in ['user', 'human']:
                    ...
                    tokens += tkns
                    masks += [1] * len(tkns)
        elif "mistral" in self.args.model_name_or_path.lower():
            for midx, message in enumerate(messages):
                if message['role'].lower() in ['user', 'human']:
                    # make sure chat_template is set to none. Otherwise, it will add <s> and </s> to the beginning and end of the text.
                    # 不需要手动加 <s>，tokenizer 会自己加
                    tkns = self.encode(f"[INST] {message['content']} [/INST]\n")
                    tokens += tkns
                    masks += [0] * len(tkns)
                else:
                    # assistant 最后一轮加self.tokenizer.eos_token_id: </s>
                    tkns = self.encode(message["content"] + "</s>")[1:] #+ [self.tokenizer.eos_token_id]
                    tokens += tkns
                    masks += [1] * len(tkns)
        # elif "mistral" in self.args.model_name_or_path.lower():
        #     for midx, message in enumerate(messages):
        #         if message['role'].lower() in ['user', 'human']:
        #             tkns = self.encode(f'<s>[INST] {message["content"]} [/INST] ')
        #             tokens += tkns
        #             masks += [0] * len(tkns)
        #         else:
        #             tkns = self.encode(message["content"]+"</s>")+[self.tokenizer.eos_token_id] + self.encode('\n')
        #             tokens += tkns
        #             masks += [1] * len(tkns)
