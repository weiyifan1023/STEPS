...
# 新版本主要为了适配LLama3 and Qwen3 等最新的 apply_chat_template
# DeepSpeed Team
import argparse
import math
import os
...
class TextDataset_V2(Dataset):
    ...
    def encode(self, text: str):
        """
        将文本编码为 token IDs 列表。
        使用 add_special_tokens=False 避免自动添加 BOS/EOS 等特殊标记，
        所有特殊标记（如 BOS, EOS, 聊天模板标记）都将在 __getitem__ 中手动处理。
        """
        # 注意: 如果你的 tokenizer.encode 默认包含 BOS 且你不需要它，
        # 推荐使用 add_special_tokens=False。
        # 如果模型需要手动添加 BOS，可以自行修改。
    ...
    def __getitem__(self, idx):
        ...
        # 1. Instruct/Chat 模型格式 (Qwen-Chat, Llama 3-Instruct)
        if self.tokenizer.chat_template is not None:

            # 确保有一个 System 消息，以便 Llama 3/Qwen 等模板能正确格式化
            system_message_exists = any(m['role'].lower() == 'system' for m in messages)
            # 假设 args 中有一个控制开关，或直接在没有系统消息时插入默认
            if not system_message_exists and (
            self.args.add_system_prompt if hasattr(self.args, 'add_system_prompt') else True):
                messages.insert(0, {"role": "system", "content": "You are a helpful assistant."})

            # A) 构造 Prompt 部分 (Mask 0)
            # 包含所有历史对话，直到最后一个用户的提问，并加上助手的起始标记
            prompt_messages = messages[:-1]
            prompt_str = self.tokenizer.apply_chat_template(
                prompt_messages,
                tokenize=False,
                add_generation_prompt=True  # 确保包含助手的起始标记
            )
            prompt_tokens = self.encode(prompt_str)
            tokens.extend(prompt_tokens)
            masks.extend([0] * len(prompt_tokens))

            # B) 构造 Response 部分 (Mask 1)
            last_response_content = messages[-1]['content']
            response_tokens = self.encode(last_response_content)

            tokens.extend(response_tokens)
            masks.extend([1] * len(response_tokens))

            # C) 统一添加 EOS/EOT 标记 (Mask 1)
            # Llama 3 使用 <|eot_id|>, Qwen 使用 <|im_end|>，都需要计入损失
            if self.tokenizer.eos_token_id is not None:
                # 确保获取的是单个 EOS/EOT token ID，并计入损失
                eos_tokens = [self.tokenizer.eos_token_id]
                tokens.extend(eos_tokens)
                masks.extend([1] * len(eos_tokens))

        # 2. Base 模型 SFT 格式 (Qwen-Base, Llama 3-Base)
        else:
            # 使用标准的 ### Instruction / ### Response 格式
            ...
            for i, message in enumerate(messages):
                ...
                if role in ['user', 'human']:
                    # Instruction 部分 (Mask 0)
                    # 包含用户提问和 Response 提示符
                    instruction_str = f'### Instruction:\n{content}\n\n### Response:\n'
                    tkns = self.encode(instruction_str)
                    tokens.extend(tkns)
                    masks.extend([0] * len(tkns))  # Prompt 部分不计入损失

                elif role == 'assistant':
                    # 回复内容 (Mask 1)
                    response_tkns = self.encode(content)
                    tokens.extend(response_tkns)
                    masks.extend([1] * len(response_tkns))

                    # 在回复后添加一个换行符作为分隔符 (Mask 0)，不计入损失
                    # 除非是最后一个回复
                    if i < len(messages) - 1:
                        tkns = self.encode('\n')
                        tokens.extend(tkns)
                        masks.extend([0] * len(tkns))

            # 在整个序列的末尾添加 EOS (强制终止)
            if self.tokenizer.eos_token_id is not None:
                # EOS 计入损失，让模型学习在生成结束后输出此标记 (Mask 1)
                final_eos_tokens = [self.tokenizer.eos_token_id]
                tokens.extend(final_eos_tokens)
                masks.extend([1] * len(final_eos_tokens))

        # 统一处理截断 (从序列尾部开始截断)
        tokens = tokens[-self.args.max_seq_len:]
        masks = masks[-self.args.max_seq_len:]
...
def main():
    ...
    wandb.login(key="089c72b152ea95e5d6ab8d48c40f5eac1bc3bbd9")
    if args.global_rank == 0:
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, shuffle=False)  # concept learning
